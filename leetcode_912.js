...
var selectionSort = function(arr) {
    ...
    for (let i = 0; i < arr.length - 1; i++) { // i 是进行到倒数第二个元素。
        let minIndex = i;
        let min = arr[i];
        for (let j = i + 1; j < arr.length; j++) { // 注意这个j的范围，j = i+1,也就是数组最后一个值也要参与比较。
            if (arr[j] < min) {
                min = arr[j];
                minIndex = j;
            }
...
var insertionSort = function(arr) {
    ...
    for (let i = 1; i < arr.length; i++) { // 最后一个数字需要比较，但是第一个数字比较的对象。
        for (let j = i - 1; j >= 0; j--) {
            if (arr[j] > arr[j + 1]) {
                [arr[j], arr[j + 1]] = [arr[j + 1], arr[j]];
            }
